...
int main(int argc, char* argv[]){
    ...
    unsigned char** m = (unsigned char**)malloc(dim * sizeof(unsigned char*));
    for(int i = 0 ; i < dim ; i++){
   ...
   // LEER FICHERO 
    ...
    for(int i = 0 ; i < dim ; i++) fread(m[i], sizeof(unsigned char), dim, f);
    fclose(f);
    ...
    for(int i = 0 ; i < dim ; i++){
        ...
        des = 0;
        for(int j = 0 ; j < dim ; j++){
            des += (m[i][j] - v_filas[i]) * (m[i][j] - v_filas[i]);
        }
